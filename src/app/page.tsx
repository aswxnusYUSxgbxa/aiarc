    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-12 border-b border-gray-200 pb-8">
          ...
          <p className="text-xl text-gray-900 max-w-2xl">Read our latest articles, insights, and humanized thoughts generated and refined for perfection.</p>
        </div>
        {posts.length === 0 ? (
          <div className="text-center py-20 text-gray-900 text-lg">No published posts yet.</div>
        ) : (
            ...
            {posts.map((post) => (
              <article key={post.id} className="flex flex-col items-start justify-between bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
                ...
                <div className="p-6 flex-1 flex flex-col justify-between w-full">
                  <div>
                    <div className="flex items-center gap-x-4 text-xs"><time dateTime={post.createdAt.toISOString()} className="text-gray-900">{format(new Date(post.createdAt), "MMMM d, yyyy")}</time></div>
                  ...
                  <div className="relative mt-8 flex items-center gap-x-4">
                    <div className="h-10 w-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-900 font-bold">{(post.authorName || "E")[0].toUpperCase()}</div>
                    <div className="text-sm leading-6"><p className="font-semibold text-gray-900">{post.authorName || "Editor"}</p><p className="text-gray-900">Author</p></div>
                  </div>
                </div>
              </article>
            ))}
